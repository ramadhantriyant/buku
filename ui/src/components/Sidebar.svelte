...
<aside class="w-64 shrink-0 flex flex-col bg-base-100 border-r border-base-200 min-h-screen">
  ...
  <nav class="flex-1 px-3 py-2 overflow-y-auto">
    ...
    <button
      class="w-full flex items-center gap-2.5 px-2 py-2 rounded-lg text-sm transition-colors
        {!selectedCategory
          ? 'bg-violet-50 text-violet-700 font-medium dark:bg-violet-950/30 dark:text-violet-300'
  ...
  <div class="px-4 pb-4 border-t border-base-200 pt-3">
    ...
    {:else}
      <button
        class="w-full flex items-center justify-center gap-2 btn btn-sm btn-ghost border border-dashed border-base-300 text-base-content/50 hover:border-violet-400 hover:text-violet-600"
