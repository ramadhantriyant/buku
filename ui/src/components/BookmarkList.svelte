<script lang="ts">
  ...
  async function deleteURL(url: Bookmark) {
    if (!confirm(`Delete bookmark "${getDomain(url.url)}"?`)) return;
...
{:else}
  <div class="grid grid-cols-3 gap-4 items-start">
    {#each urls as url (url.id)}
      <div class="group bg-base-100 rounded-xl border border-base-200 hover:border-green-300 hover:shadow-lg transition-all flex flex-col">
        ...
        <div class="p-4 pb-3">
          <div class="flex items-start gap-3">
            ...
            <!-- Actions -->
            <div class="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0 -mr-1 -mt-1">
              <button
        ...
        <!-- Description - Variable height based on content -->
        {#if url.description}
        ...
        <div class="px-4 py-3 border-t border-base-200 bg-base-50/50 rounded-b-xl">
          <div class="flex items-center justify-between">
            <span class="badge badge-sm text-xs font-medium bg-green-800 text-white border-0">{getCategoryName(url.category_id)}</span>
